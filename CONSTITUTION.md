...
4. **Single-Source Design Tokens**: The shared palette lives once as CSS custom properties in the `:root` block of `globalCss` (`App.jsx`) — paper, kraft, vermilion, leaf, slate, ink, sand, tape. Stylesheets consume `var(--token)`; no new hard-coded hex for a shared palette color, and no class may be defined in more than one stylesheet (duplicate definitions make the rendered style depend on stylesheet injection order).
